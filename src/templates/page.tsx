...
export default ({ data }: Props) => (
  <Layout>
    ...
    <Helmet>
      ...
      <meta
        property="og:url"
        content={`https://opontolaranja.pt/${data.ghostPage.slug}/`}
      />
      <meta property="og:type" content="article" />
      <meta
        property="og:description"
        content={truncate(data.ghostPage.excerpt, 200)}
      />
      <link
        rel="canonical"
        href={`https://opontolaranja.pt/${data.ghostPage.slug}/`}
      />
    </Helmet>

    {data.ghostPage.feature_image ? (
      <ImageMeta image={data.ghostPage.feature_image} />
    ) : null}
